  ...
  If ($identification) {
    Try {
      If ($identification -match "^(CN|DC|OU)=") {
        ...
        # samaccountname
        $result = Get-ADObject -Filter {samaccountname -eq $identification} `
                               @extra_args
      }
...
Function Get-CanonicalDistinguishedName($distinguishedName) {
  $result = ($distinguishedName -split '(,?[a-z]+=)' | Foreach-Object {
    If ($_ -Match '(,?[a-z]+=)') {
      $_.ToUpper()
    } Else {
        $_
...
$protected_from_accidental_deletion = `
  $module.Params.protected_from_accidental_deletion
...
if ($state -ne 'absent')
{
  if ($recursive -ne $false) {
    $module.FailJson(
      "Parameter 'recursive' must be False when state='$state'", $_)
  }
}
...
if ($state -ne 'present')
{
  if ($description -ne $null) {
    $module.FailJson(
      "Parameter 'description' must be empty when state='$state'", $_)
  }
  if ($managed_by -ne $null) {
    $module.FailJson(
      "Parameter 'managed_by' must be empty when state='$state'", $_)
  }
  if ($protected_from_accidental_deletion -ne $null) {
    $module.FailJson("Parameter 'protected_from_accidental_deletion' must " +
                     "be empty when state='$state'", $_)
  }
}
...
Function Get-ActualState($desired_state) {
  ...
  If ($ou -ne $null) {
    $initial_state = [ordered]@{
      ...
      path = Get-CanonicalDistinguishedName (
        $ou.DistinguishedName -creplace "^[^,]*,","")
...
Function Set-ConstructedState($initial_state, $desired_state) {
  ...
  Foreach ($property_in_ps_command in $ou_properties.Keys) {
    ...
    If ($initial_state[$property] -ne $desired_state[$property]) {
        ...
        Try {
          Set-ADOrganizationalUnit -Identity $ou_full_path `
            -WhatIf:$module.CheckMode @set_args
        } Catch {
          $module.FailJson("Failed to set the OU property " +
            "$ou_full_path[$property]: $($_.Exception.Message)", $_)
        }
  ...
  Try {
    New-ADOrganizationalUnit `
      ...
      -ProtectedFromAccidentalDeletion `
        $desired_state.protected_from_accidental_deletion `
      -WhatIf:$module.CheckMode `
      @extra_args
    } Catch {
      $module.FailJson("Failed to create the OU ${ou_full_path}: " +
...
Function Remove-ConstructedState($recursive) {
  ...
  Try {
    ...
    $module.FailJson("Failed to remove the OU ${ou_full_path}: " +
                     "$($_.Exception.Message)", $_)
  }
  $module.Result.changed = $true
}
...
If ($desired_state.state -eq "present") {
    If ($initial_state.state -eq "present") {
      $in_desired_state = (ConvertTo-SerializedState $initial_state) `
                           -eq (ConvertTo-SerializedState $desired_state)
